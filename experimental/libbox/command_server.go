...
type CommandServer struct {
	...
	// These hooks only work with a single client. if multi-client support is needed, replace with Subscriber/Observer
	urlTestUpdate *observable.Subscriber[struct{}]
	modeUpdate    *observable.Subscriber[struct{}]
...
func NewCommandServer(handler CommandServerHandler, maxLines int32) *CommandServer {
	server := &CommandServer{
		...
		urlTestUpdate: observable.NewSubscriber[struct{}](1),
		modeUpdate:    observable.NewSubscriber[struct{}](1),
